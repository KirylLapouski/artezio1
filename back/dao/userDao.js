...
var UserDao = {
    ...
    read: (callback, id) => {
        return User.find(typeof id == "string" ? {
            _id: id
        } : id).then((currentUser, err) => {
    ...
    readSmallVersion: (callback) => {
        User.find().select('email firstName lastName phoneNumber city facebookId linkedinId').then((currentUser, err) => {
            currentUser = currentUser ? (currentUser.length == 1 ? currentUser[0] : currentUser) : currentUser;
            if (callback)
                callback(err, currentUser);
            return currentUser;
